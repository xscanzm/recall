## v0.5.12 — 视觉链路降级保底：上游不可用时记忆管线不停摆

本次版本解决免费上游容量崩塌导致记忆系统"断粮"的问题：默认模型服务的视觉链路不可用（限流/超时）时，L0 观察自动降级为由本地 OCR + 窗口元数据生成（零模型调用），观察→Episode→事实的记忆管线持续运转；上游恢复后自动回升。用户自配视觉 API 完全不受影响。

### 新增

#### 1. 视觉链路 OCR 降级保底（默认模型服务）

- 默认服务视觉上游不可用（`rate_limited` / 超时类错误）时，当前批次立即降级：用采集时已生成的本地 OCR 结果（RapidOCR / Windows OCR，零额外成本）+ 窗口元数据构造 L0 观察，批次不再整批丢失
- 熔断器自动管理：首次失败即熔断，冷却 5 分钟起步、连续失败指数延长（30 分钟封顶），冷却结束自动放行探测批次，成功即恢复视觉链路；应用重启时按最近 30 分钟默认服务任务结果水合，不重复空烧已知饱和的上游
- 降级期间 Episode 切分、事实抽取、场景关联照常工作（文本链路不受视觉上游影响），记忆时间线不断档
- **用户自配视觉 API 不参与降级**：只要设置里配置了自己的视觉 API，永远走原有视觉链路，不降级、不熔断，行为与之前完全一致

#### 2. L0 观察溯源（迁移 031）

- observations 表新增 `generation_path` 列，记录每条 L0 观察的生成来源：`vision_model:v1`（视觉模型）或 `ocr_fallback:v1`（降级 OCR），历史行为 NULL
- 为后续"视觉恢复后回填升级"（把降级观察重跑为视觉观察）提供数据基础
- 迁移在应用首次启动时自动执行，执行前自动备份数据库

### 改进

#### 1. 上游 429 原因透明化

- 默认服务上游返回 429 时，调试页错误详情保留上游真实响应体（如 "Server is busy, please try again later"），不再只有笼统的"上游请求被限流"，便于区分配额问题与容量饱和

#### 2. 免费上游限流适配（服务端，已先行部署）

- Worker 异步任务队列消费并发 3→1，429 重试退避 30s→90s，避免重试风暴加剧免费上游饱和、给上游恢复留出排空时间

### 已知限制

- 降级观察由本地 OCR 生成，无场景语义理解：置信度较低（0.15-0.35），视频/画布类窗口可能只有窗口标题信息
- 降级模式不做视觉隐私评估（privacyRisk 如实标注为 medium）；采集前的隐私保护规则不受影响
- 免费上游恢复时间取决于 SenseNova 渠道容量，视觉恢复后新批次自动走回视觉链路

## v0.5.11 — 模型管线健壮性修复

本次版本聚焦模型管线的健壮性：修复记忆批量任务因模型输出缺字段导致的失败并强化修复机制，让默认模型服务的上游超时可控可感知，并让本地轮询超时可以被安全重试，不再丢失任务结果。

### 修复

#### 1. 记忆批量任务批量失败修复

- 模型输出缺少 `facts` / `observations` 字段时不再触发 schema 校验失败，归一化层自动补 `[]` 兜底
- 增强 repair 修复机制：repair 调用使用双倍输出预算（`max_tokens` 2×），并强化 prompt 对字符串内引号转义的要求，修复"输出被截断"与"引号未转义"两类 JSON 解析失败

### 改进

#### 1. 默认模型服务上游超时可控化

- 非流式请求增加 60s 显式超时，超时返回明确错误（`504` / `upstream_timeout`），不再无限挂起
- 流式请求不受影响（长连接不被误切断）

#### 2. 本地轮询超时可安全重试

- 本地轮询超时不再误判为不可重试的上游超时，拆分独立错误码 `async_poll_timeout`，允许安全重试，避免任务结果永久丢失

## v0.5.10 — 修复：移除 Worker 默认模型代理按 IP 限流

本次版本为补丁修复：移除 Worker 默认模型代理（语言/多模态）按 IP 的每日限流，解决 v0.5.9 引入的限流过严问题。该限流基于 D1 原子计数（默认 200 次/天/IP），导致桌面端批量任务在 8月3日-8月5日出现大量 `rate_limited - 上游代理限流 (HTTP 429)` 错误（452+ 次），`batch_pipeline` 任务持续失败（retry_pending / retry_exhausted）。

### 修复

#### 1. 移除 Worker 默认模型代理按 IP 限流

- 移除语言/多模态模型代理按客户端 IP 的每日限流（D1 原子计数，默认 200 次/天/IP），服务端不再做任何限流
- 同时移除信息图代理（`/api/infographic/generate`）按 IP 每日 100 次的 KV 计数限制
- 新增 D1 迁移 `0005` 清理 `model_proxy_rate_limits` 表
- 客户端对任意上游 429 的弹性重试逻辑保留不变

## v0.5.9 — 安全加固 + Electron 43 升级 + 渲染层健壮性

本次版本以安全加固为主线：收紧更新安装、IPC 与更新下载链路的安全边界；同时升级 Electron 至 43.2.0 并启用 ASAR 完整性校验，补齐隐私数据一致性、数据库查询护栏与渲染层健壮性，并恢复测试覆盖率真实口径。
...
#### 2. Worker 默认模型代理按 IP 限流（已移除）

- 基于 D1 原子计数实现按客户端 IP 每日限流，状态轮询请求豁免，超限返回 `429` + `Retry-After`
- > ⚠️ 该限流默认 200 次/天/IP 过严，导致桌面端批量任务大量 429（`rate_limited - 上游代理限流`）；服务端限流已彻底移除，客户端仍保留对任意上游 429 的弹性重试。
