...
> 下载后可校验完整性：`certutil -hashfile Recall-0.5.2-setup.exe SHA256`，与 [GitHub Release](https://github.com/xscanzm/recall/releases/latest) 页面公布的 SHA-256 比对。
...
2. 双击安装 `Recall-0.5.2-setup.exe`，桌面会出现 **Recall** 图标。
...
> 构建产物 `release/Recall-0.5.2-setup.exe` 即对应发布通道的分发物。
