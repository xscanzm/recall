...
- **本地优先 + 隐私边界**：截图与数据库默认保存在本地；自带 API Key 时模型调用直连你自己的 endpoint。但 Recall 也内置远程服务（默认模型代理 `recall-update.ppclaw.online/api/model/*`、信息图生成代理、更新检查/下载分发）——当使用内置的「Recall 默认模型服务」（未自配 endpoint）时，提示词与截图会以请求数据形式经 Recall 的代理服务器转发，异步多模态任务请求体（含截图 data URL）会临时存储于 Recall 的 Cloudflare R2，最长保留 2 小时后自动删除。
- **可审计开源**：完整源码公开（BUSL 1.1），无后门；内置远程服务的行为（见下方「隐私与信任声明」）均在源码中可审计。
...
> 开箱即用：默认使用 Recall 内置的「默认模型服务」（模型调用经 Recall 代理转发，见下方「隐私与信任声明」）；也可自备 OpenAI-compatible 端点的 API Key，此时模型调用直连你自己的 endpoint。Recall 内置的远程服务包括默认模型代理、信息图生成代理与更新检查/下载分发。
...
Recall 在以下场景收集有限遥测数据：使用内置的「Recall 默认模型服务」时，客户端会上报随机安装标识（UUID）、任务类型、客户端版本，以及按安装统计的调用量、成功与失败次数；服务端对安装标识做 HMAC 哈希后存储，不含提示词、截图等任何内容。使用该默认服务时，截图会作为请求图片数据上传至 Recall 代理服务器；使用自配 endpoint 时不上传截图，也不产生上述遥测。

- 截图仅保存在用户本地 `appData` 缓存目录，文件名不含窗口标题或 URL。
- API Key 通过系统级安全存储（Windows Credential Manager），不进 SQLite。
- 模型调用：自配 endpoint 时直连你自己的服务器，流量不经 Recall 中转；使用「Recall 默认模型服务」时，调用经由 Recall 的代理服务器转发。
